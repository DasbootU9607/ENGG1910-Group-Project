...
\section{Data Requirement and Preparation}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{figures/label_distribution.png}
\caption{Original LIAR label distribution on the official test split.}
\end{figure}

\section{Application Design and AI Method}
...
The learning stage predicts probabilities for Low, Medium, and High risk. The displayed risk score is computed as \texttt{0.5 * prob\_medium + prob\_high}, so Medium risk contributes partial concern and High risk contributes full concern. For the formal LIAR experiment, predictions are selected by multiclass argmax. We report macro precision, macro recall, macro-F1, weighted-F1, and multiclass one-vs-rest macro ROC-AUC.
...
\bottomrule
\end{tabular}
\caption{Test-set comparison under the fair three-class LIAR protocol.}
\end{table}

\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{figures/model_comparison.png}
\caption{Model comparison across multiclass classification metrics.}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{figures/roc_curves.png}
\caption{One-vs-rest ROC curves for the fair text-only models.}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{figures/risk_distribution.png}
\caption{Predicted three-level risk distribution from BERT-base.}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{figures/baseline_confusion_matrix.png}
\caption{Three-class confusion matrix for TF-IDF text-only logistic regression.}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{figures/transformer_confusion_matrix.png}
\caption{Three-class confusion matrix for BERT-base text-only fine-tuning.}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{figures/score_distribution.png}
\caption{Distribution of continuous risk scores computed from Medium and High probabilities.}
\end{figure}
...
[3] Hugging Face. ucsbnlp/liar Dataset Repository. \url{https://huggingface.co/datasets/ucsbnlp/liar}

[4] PolitiFact. Fact-checking political claims. \url{https://www.politifact.com/}
