...
This project designs an AI-powered misinformation risk assistant for social media users. Instead of directly judging whether a post is true or false, the system estimates whether a post has a higher risk of containing misleading information and explains the warning signals behind the score. The prototype uses natural language processing and metadata-based features to analyze short claims. We tested the idea on the LIAR dataset, a public benchmark containing 12.8K manually labelled political statements from PolitiFact. In our implementation, LIAR's six truthfulness labels were mapped into a binary risk task: `false`, `barely-true`, and `pants-fire` were treated as higher-risk claims, while `half-true`, `mostly-true`, and `true` were treated as lower-risk claims. A logistic regression model using TF-IDF text features and speaker history features achieved 0.743 accuracy, 0.712 F1 score, and 0.831 ROC-AUC on the LIAR test set. The result suggests that AI can support users by prioritizing posts for checking, but it should not replace human judgment or professional fact-checking.
...
Third, the learning module uses logistic regression. This model is suitable for a course project because it is efficient, interpretable, and works well with sparse TF-IDF features. It outputs a probability between 0 and 1. A higher probability means the post is more likely to require careful checking.
...
On the LIAR test set, the prototype achieved an accuracy of 0.743, precision of 0.692, recall of 0.732, F1 score of 0.712, and ROC-AUC of 0.831. The confusion matrix contained 546 true negatives, 181 false positives, 149 false negatives, and 407 true positives. These results show that the model can identify many higher-risk claims, but still makes both types of mistakes.
